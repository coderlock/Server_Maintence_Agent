...
import { validateIpcInput, AppSettingsUpdateSchema, ApiKeySchema } from './ipcSchemas';
...
export function registerSettingsHandlers(): void {
  ...
  ipcMain.handle(IPC_CHANNELS.SETTINGS.UPDATE, async (_event, updates: unknown) => {
    try {
      const safeUpdates = validateIpcInput(AppSettingsUpdateSchema, updates) as Partial<AppSettings>;
      const updated = await settingsStore.updateSettings(safeUpdates);

      // If provider changed, switch the orchestrator's active provider
      if (safeUpdates.aiProvider) {
        aiOrchestrator.setProvider(safeUpdates.aiProvider);
      }

      // If model changed, apply it to the correct provider
      if (safeUpdates.aiModel) {
        const settings = await settingsStore.getSettings();
        const providerName = safeUpdates.aiProvider ?? settings.aiProvider ?? 'moonshot';
  ...
  ipcMain.handle(IPC_CHANNELS.SETTINGS.SET_API_KEY, async (_event, apiKey: unknown) => {
    try {
